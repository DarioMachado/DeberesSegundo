...
public class GUI extends JFrame {
    ...
    private String[] palabras = {"JAVA", "PERRO", "GATO", "CENEC", "DINOSAURIO", "TECLADO"};
    ...
    public GUI(){
        ...
        JPanel panel = new JPanel(){
            ...
            protected void paintComponent(Graphics g) {
                ...
                g2d.drawString(imprimirPalabra(palabra, dichas),300,100);
        ...
        while(true) {
            turno++;
            System.out.println(imprimirAhorcado(errores));
            System.out.println(imprimirPalabra(palabra, dichas));
            if(errores>5) {
            ...
            while(true) {
                input = sca.nextLine();
                if(input.length()==1 && (int)input.toUpperCase().charAt(0) > 64 && (int) input.toUpperCase().charAt(0) < 91)
                    break;
                else
                    System.out.println("Ha habido un error, inténtalo otra vez.");
            }
            input = input.toUpperCase();
            if(palabra.contains(input)&&!dichas.containsValue(input)) {
                System.out.println("La letra está.");
                dichas.put(turno, input);
                if (palabra.equals(imprimirPalabra(palabra, dichas))) {
                    System.out.println("¡Has ganado!");
                    break;}
                continue;
            }
            else if(!palabra.contains(input))
                System.out.println("La letra no está.");
            else if(dichas.containsValue(input)) {
